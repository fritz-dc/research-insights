...
# ── 12A. Evidence-unit mechanism discovery helpers (NB03 optional) ───────────

DEFAULT_MECHANISM_DISCOVERY_CONFIG: dict[str, Any] = {
    "enabled": False,
    "ngram_range": [1, 3],
    "min_phrase_doc_count": 20,
    "min_lift_vs_rest": 1.4,
    "max_phrase_doc_share": 0.65,
    "max_phrases_per_group": 150,
    "cooccurrence_jaccard_threshold": 0.08,
    "min_unit_phrases": 2,
    "min_unit_projects": 50,
    "max_units_per_group": 8,
    "max_terms_per_unit": 12,
    "max_projects_per_unit": 1000,
    "min_token_len": 3,
    "exclude_terms": [],
    "exclude_tokens": [],
}

DEFAULT_MECHANISM_GENERIC_TOKENS: set[str] = {
    "student", "students", "teacher", "teachers", "school", "classroom",
    "class", "project", "projects", "learn", "learning", "education",
    "need", "needs", "help", "support", "supports", "resource", "resources",
    "material", "materials", "supply", "supplies", "use", "used", "using",
    "make", "made", "give", "work", "works", "working", "grade", "grades",
    "day", "days", "year", "years", "time", "times", "new", "many",
    "every", "each", "one", "two", "also", "able", "would", "could",
    "want", "wants", "allow", "allows", "provide", "provides", "create",
    "build", "building", "activity", "activities", "experience", "experiences",
    "opportunity", "opportunities", "skills", "skill", "tool", "tools",
}


def get_evidence_generation_config(cfg: dict[str, Any]) -> dict[str, Any]:
    """Return the optional NB03 evidence-generation config.

    The existing NMF path remains the default. This helper normalizes a small
    config surface so notebooks can keep orchestration simple while utils.py
    owns mechanism-discovery defaults.
    """
    eg = cfg.get("evidence_generation", {}) or {}
    method = str(eg.get("method", "nmf")).strip().lower()
    mechanism_cfg = {
        **DEFAULT_MECHANISM_DISCOVERY_CONFIG,
        **(eg.get("mechanism_discovery", {}) or {}),
    }
    # Allow method to turn mechanism discovery on without repeating enabled:true.
    if method in {"mechanism_discovery", "mechanism", "hybrid"}:
        mechanism_cfg["enabled"] = bool(mechanism_cfg.get("enabled", True))
    else:
        mechanism_cfg["enabled"] = bool(mechanism_cfg.get("enabled", False))
    return {"method": method, "mechanism_discovery": mechanism_cfg}


def _mechanism_clean_token(token: Any, *, min_token_len: int, excluded: set[str]) -> str:
    """Normalize one token for local mechanism discovery."""
    t = str(token or "").strip().lower()
    if not t or t.startswith("__"):
        return ""
    if len(t) < min_token_len:
        return ""
    if t in excluded or t in DEFAULT_MECHANISM_GENERIC_TOKENS:
        return ""
...
def _mechanism_project_phrases(
    tokens: Any,
    *,
    ngram_range: tuple[int, int],
    min_token_len: int,
    ...
    """Return unique n-gram phrases for one project's token list."""
    toks = [
        _mechanism_clean_token(t, min_token_len=min_token_len, excluded=excluded_tokens)
        for t in coerce_token_list(tokens)
    ]
    ...
    phrases: set[str] = set()
    for n in range(max(1, min_n), max(1, max_n) + 1):
        if len(toks) < n:
            continue
        for i in range(len(toks) - n + 1):
            phrase = " ".join(toks[i:i + n])
            if phrase and phrase not in excluded_terms:
                phrases.add(phrase)
    return phrases


def _mechanism_connected_components(
    phrase_sets: dict[str, set[Any]],
    *,
    jaccard_threshold: float,
) -> list[list[str]]:
    """Cluster phrases by shared supporting-project sets using connected components."""
    phrases = list(phrase_sets.keys())
    adj: dict[str, set[str]] = {p: set() for p in phrases}
    for i, a in enumerate(phrases):
        ...
        for b in phrases[i + 1:]:
            ...
            if not inter:
                continue
            jac = inter / max(1, len(set_a | set_b))
...
def _mechanism_label_from_terms(terms: list[str], max_terms: int = 3) -> str:
    """Deterministic placeholder label for a local mechanism unit."""
    shown = [str(t).strip() for t in terms if str(t).strip()][:max_terms]
    return " / ".join(shown) if shown else "local mechanism bundle"


def build_mechanism_evidence_units(
    df: pd.DataFrame,
    *,
    groupby_field: str,
    ...
    project_id_col: str = "project_id",
    token_col: str = "tokens",
    existing_topics_df: pd.DataFrame | None = None,
) -> dict[str, pd.DataFrame]:
    """Build local mechanism evidence units for NB03 without per-project LLM calls.

    The output intentionally mirrors the NMF topic artifacts so the existing
    NB03 labeling, synthesis, verification, evidence, packaging, and reporting
    path can remain unchanged. Mechanism bundles are represented as additional
    pseudo-topics with stable, non-colliding topic_id values per group.

    Method:
      1. Build project-level n-gram phrase sets from the token column.
      2. Score phrases within each group against the rest of the current run
         using document-share lift.
      3. Cluster distinctive phrases by project co-occurrence.
      4. Emit topic-shaped rows, weight rows, and bridge rows.

    This is local and deterministic. Optional LLM labeling happens later through
    the existing topic-labeling step, only once per evidence unit.
    """
    if df.empty:
        return {
            ...
            "phrase_scores_df": pd.DataFrame(),
            "unit_phrase_membership_df": pd.DataFrame(),
            "summary_df": pd.DataFrame(),
        }

    cfg = {**DEFAULT_MECHANISM_DISCOVERY_CONFIG, **(cfg or {})}
    min_phrase_doc_count = int(cfg.get("min_phrase_doc_count", 20))
    min_lift_vs_rest = float(cfg.get("min_lift_vs_rest", 1.4))
    max_phrase_doc_share = float(cfg.get("max_phrase_doc_share", 0.35))
    max_phrases_per_group = int(cfg.get("max_phrases_per_group", 150))
    jaccard_threshold = float(cfg.get("cooccurrence_jaccard_threshold", 0.08))
    min_unit_phrases = int(cfg.get("min_unit_phrases", 2))
    min_unit_projects = int(cfg.get("min_unit_projects", 50))
    max_units_per_group = int(cfg.get("max_units_per_group", 8))
    max_terms_per_unit = int(cfg.get("max_terms_per_unit", 12))
    max_projects_per_unit = int(cfg.get("max_projects_per_unit", 1000))
    min_token_len = int(cfg.get("min_token_len", 3))
    raw_ngram_range = cfg.get("ngram_range", [1, 3])
    ngram_range = (int(raw_ngram_range[0]), int(raw_ngram_range[1]))
    excluded_terms = {str(x).strip().lower() for x in (cfg.get("exclude_terms", []) or []) if str(x).strip()}
    excluded_tokens = {str(x).strip().lower() for x in (cfg.get("exclude_tokens", []) or []) if str(x).strip()}
    excluded_tokens |= DEFAULT_MECHANISM_GENERIC_TOKENS

    work = df[[project_id_col, groupby_field, token_col]].dropna(subset=[project_id_col, groupby_field]).copy()
    # A project may appear once per strategic_injected_tag group; keep one row
    # per project × group for local evidence-unit discovery.
    work = work.drop_duplicates([project_id_col, groupby_field]).reset_index(drop=True)
    if work.empty:
        return {
            "topics_df": pd.DataFrame(),
            "weights_df": pd.DataFrame(),
    ...
    work["_phrases"] = work[token_col].apply(
        lambda toks: _mechanism_project_phrases(
            toks,
            ngram_range=ngram_range,
    ...
    group_sizes = work.groupby(groupby_field)[project_id_col].nunique().to_dict()
    total_group_docs = int(sum(group_sizes.values()))

    group_phrase_projects: dict[Any, dict[str, set[Any]]] = {}
    total_phrase_count: dict[str, int] = defaultdict(int)
    for group, sub in work.groupby(groupby_field, observed=True):
        phrase_projects: dict[str, set[Any]] = defaultdict(set)
        for _, row in sub.iterrows():
            pid = row[project_id_col]
            for phrase in row["_phrases"]:
                phrase_projects[phrase].add(pid)
        group_phrase_projects[group] = dict(phrase_projects)
        for phrase, pids in phrase_projects.items():
            total_phrase_count[phrase] += len(pids)

    start_topic_id: dict[Any, int] = defaultdict(int)
    if existing_topics_df is not None and not existing_topics_df.empty:
        for group, sub in existing_topics_df.groupby(groupby_field, observed=True):
            try:
                start_topic_id[group] = int(pd.to_numeric(sub["topic_id"], errors="coerce").max()) + 1
            except Exception:
                start_topic_id[group] = 10000
    ...
    for group, phrase_projects in group_phrase_projects.items():
        group_n = int(group_sizes.get(group, 0))
        rest_n = max(0, total_group_docs - group_n)
        if group_n <= 0:
            continue

        phrase_scores = []
        for phrase, pids in phrase_projects.items():
            group_count = len(pids)
            if group_count < min_phrase_doc_count:
                continue
            group_share = group_count / group_n
            if group_share > max_phrase_doc_share:
                continue
            rest_count = max(0, int(total_phrase_count.get(phrase, 0)) - group_count)
            rest_share = rest_count / rest_n if rest_n else 0.0
            lift = (group_share + 1e-6) / (rest_share + 1e-6)
            if lift < min_lift_vs_rest:
                continue
            # Weighted score balances prevalence and distinctiveness.
            score = float(np.log1p(group_count) * np.log1p(lift))
            phrase_scores.append({
                "group_value": group,
                "phrase": phrase,
                "group_doc_count": int(group_count),
                "group_doc_share": float(group_share),
                "rest_doc_count": int(rest_count),
                "rest_doc_share": float(rest_share),
                "lift_vs_rest": float(lift),
                "score": score,
            })
        ...
        if not phrase_scores:
            summary_rows.append({
                "group_value": group,
                "group_project_count": group_n,
                "candidate_phrase_count": 0,
                "unit_count": 0,
                "skip_reason": "no_candidate_phrases",
            })
            continue

        phrase_scores = sorted(phrase_scores, key=lambda r: (-r["score"], -r["group_doc_count"], r["phrase"]))[:max_phrases_per_group]
        score_rows.extend(phrase_scores)
        candidate_phrases = [r["phrase"] for r in phrase_scores]
        phrase_score_lookup = {r["phrase"]: r for r in phrase_scores}
        candidate_sets = {p: phrase_projects[p] for p in candidate_phrases}
        comps = _mechanism_connected_components(candidate_sets, jaccard_threshold=jaccard_threshold)

        unit_candidates = []
        for comp in comps:
            comp_sorted = sorted(comp, key=lambda p: (-phrase_score_lookup[p]["score"], -len(candidate_sets[p]), p))
            if len(comp_sorted) < min_unit_phrases:
                # Allow a strong singleton phrase only when it has substantial support.
                if len(candidate_sets[comp_sorted[0]]) < max(min_unit_projects * 2, min_phrase_doc_count):
                    continue
            top_terms = comp_sorted[:max_terms_per_unit]
            support_counter: dict[Any, int] = defaultdict(int)
            for phrase in top_terms:
                for pid in candidate_sets[phrase]:
                    support_counter[pid] += 1
            support_items = sorted(support_counter.items(), key=lambda kv: (-kv[1], str(kv[0])))
            support_pids = [pid for pid, _ in support_items]
            if len(support_pids) < min_unit_projects:
                continue
            unit_score = float(sum(phrase_score_lookup[p]["score"] for p in top_terms))
            unit_candidates.append({
                "top_terms": top_terms,
                "support_items": support_items[:max_projects_per_unit],
                "support_count": len(support_pids),
                "score": unit_score,
            })

        unit_candidates = sorted(unit_candidates, key=lambda r: (-r["score"], -r["support_count"]))[:max_units_per_group]
        next_topic_id = int(start_topic_id.get(group, 0))
        for unit_idx, unit in enumerate(unit_candidates):
            topic_id = next_topic_id + unit_idx
            top_terms = unit["top_terms"]
            support_items = unit["support_items"]
            label = _mechanism_label_from_terms(top_terms)
            topic_rows.append({
                groupby_field: group,
                "topic_id": int(topic_id),
                "top_terms": top_terms,
                "top_weights": [float(phrase_score_lookup[p]["score"]) for p in top_terms],
                "source_method": "mechanism_discovery",
                "mechanism_unit_id": f"mechanism_{slugify_group_value(group)}_{topic_id}",
                "mechanism_label_seed": label,
                "mechanism_supporting_project_count": int(unit["support_count"]),
            })
            for phrase_rank, phrase in enumerate(top_terms, start=1):
                membership_rows.append({
                    groupby_field: group,
                    "topic_id": int(topic_id),
                    "mechanism_unit_id": f"mechanism_{slugify_group_value(group)}_{topic_id}",
                    "phrase_rank": int(phrase_rank),
                    **phrase_score_lookup[phrase],
                })
            denom = max(1, len(top_terms))
            for rank, (pid, match_count) in enumerate(support_items):
                weight = float(match_count)
                topic_share = float(min(1.0, match_count / denom))
                weight_rows.append({
                    groupby_field: group,
                    "topic_id": int(topic_id),
                    "project_id": pid,
        ...
        summary_rows.append({
            "group_value": group,
            "group_project_count": group_n,
            "candidate_phrase_count": len(phrase_scores),
            "unit_count": len(unit_candidates),
            "skip_reason": "" if unit_candidates else "no_units_after_support_filter",
        })

    return {
        "topics_df": pd.DataFrame(topic_rows),
        "weights_df": pd.DataFrame(weight_rows),
        "project_topic_bridge_df": pd.DataFrame(bridge_rows),
        "phrase_scores_df": pd.DataFrame(score_rows),
        "unit_phrase_membership_df": pd.DataFrame(membership_rows),
...
def build_per_group_prompt(
    ...
    """
    return f"""
Below is a list of evidence units discovered from teacher project request essays on DonorsChoose
for a single group: {group} ({group_description}).
Evidence units may be NMF topics, cross-group topic clusters, or locally discovered mechanism bundles.
Each unit represents essays with related language, framing, request patterns, or teacher-described mechanisms.
{topic_lines_text}
...
__all__ = [
    ...
    "nmf_one",
    "DEFAULT_MECHANISM_DISCOVERY_CONFIG",
    "get_evidence_generation_config",
    "build_mechanism_evidence_units",
    # Enrichment helpers
    "gate_cluster",
    "classify_batch",
    "inject_tokens",
...
# ── 21. Classroom Compass HTML report builder helpers
...
def cc_add_warning(
    ...
    """Append an in-memory warning to the NB04 report-builder diagnostics dict.

    This is intentionally separate from append_warning(), which writes JSONL
    warning files for the pipeline stages. NB04 uses an in-memory diagnostics
    object and writes it into the final report manifest.
    """
...
# Tier sort/display keys are stored in the same mixed-case form that
# cc_normalize_tier() returns (A, B, C, D, Unscored). This keeps lookups
# direct so the payload's category_value_add_tier values can be used as
# dict keys without case coercion.
CC_REPORT_TIER_ORDER = {"A": 1, "B": 2, "C": 3, "D": 4, "Unscored": 5}
CC_REPORT_TIER_DISPLAY = {"A": "A", "B": "B", "C": "C", "D": "D", "Unscored": "Unscored"}


def cc_safe_float(value: Any, default: float = 0.0) -> float:
    """Convert a value to finite float, with safe fallback for null/NaN strings."""
    try:
        if value is None:
            return default
...
def cc_report_slug(value: Any, max_len: int = 80) -> str:
    """Return a compact ID-safe slug used by NB04 report artifacts."""
    if value is None:
        s = ""
...
def cc_first_present(row: pd.Series | dict[str, Any], candidates: list[str], default: Any = "") -> Any:
    """Return the first non-empty value from row[candidates]."""
...
def cc_make_global_insight_id(row: pd.Series, idx: int = 0) -> str:
    """Create the global insight ID used by sweep-review tier files.

    Contract: byte-for-byte identical to the union/ranking script that
    builds OUTPUTS/sweep_review/insight_value_add_tiers.csv. If you change
    either side, change both, or this join silently drops to Unscored.

    Recipe:
        slug(strategic_area_id)__slug(split_id)__slug(category_bucket, 50)
        __insight_part__slug(title, 70)

    where insight_part = slug(insight_id) if a non-empty insight_id is
    present, otherwise the deterministic fallback "row_{idx:06d}".

    Notes:
        - split_id falls back to the literal string "split" when missing.
          Non-strategic runs have split_id=None in nb03 output, so they
          land on this literal — the union script does the same. Adding
          other fields to the fallback chain (e.g. resolved_groupby_field)
          would break the join for every non-strategic insight.
        - This function does NOT short-circuit on an already-present
          global_insight_id column. The union script always recomputes,
          so we do too.
        - Duplicate handling (__dup_N suffix) is the caller's job; see
          cc_load_curated_union().
    """
    strategic_area_id = cc_first_present(row, ["strategic_area_id"], "area")
    split_id = cc_first_present(row, ["split_id"], "split")
    category_bucket = cc_first_present(row, ["category_bucket"], "")
    insight_id = cc_first_present(row, ["insight_id"], "")
    title = cc_first_present(row, ["title"], "")
    insight_part = cc_report_slug(insight_id) if insight_id else f"row_{idx:06d}"
    return "__".join(
        [
            cc_report_slug(strategic_area_id),
            cc_report_slug(split_id),
            cc_report_slug(category_bucket, max_len=50),
            insight_part,
            cc_report_slug(title, max_len=70),
        ]
    )


def cc_parse_project_ids(value: Any, max_ids: int | None = None) -> list[Any]:
    """Parse project IDs from lists, JSON/list strings, comma strings, or scalar values."""
    if value is None:
        return []
    if isinstance(value, float) and pd.isna(value):
        return []
    ...
    for v in vals:
        ...
        if key not in seen:
            seen.add(key)
            out.append(text)
        if max_ids is not None and len(out) >= int(max_ids):
            break
    return out


def cc_read_json_if_exists(path: Path) -> dict[str, Any]:
    path = Path(path)
    if not path.exists():
        return {}
    try:
...
def cc_infer_run_dir_from_curated(path: Path) -> Path:
    """Return the NB03 run directory for a curated_df.csv path."""
    path = Path(path)
    if path.parent.name in {"chart_data", "insights"}:
        return path.parent.parent
    cur = path.parent
    for parent in [cur] + list(cur.parents):
        if (parent / "metadata").exists() or (parent / "insights").exists() or (parent / "chart_data").exists():
            return parent
    return path.parent


def cc_source_root_type(path: Path) -> str:
    parts = list(Path(path).parts)
    if "strategic_area" in parts:
        return "strategic_area"
    if "non_strategic" in parts:
        return "non_strategic"
    return "unknown"


def cc_infer_path_metadata(path: Path, root: Path) -> dict[str, Any]:
    """Infer report source metadata from the NB03 output path and manifest."""
    path = Path(path)
    root = Path(root)
    run_dir = cc_infer_run_dir_from_curated(path)
    rel = path.relative_to(root) if path.is_relative_to(root) else path
    manifest = cc_read_json_if_exists(run_dir / "metadata" / "pipeline_manifest.json")
    root_type = cc_source_root_type(path)
    parts = list(path.parts)

    strategic_area_id = manifest.get("strategic_area_id") or ""
    non_strategic_groupby = ""
    run_id = manifest.get("run_id") or run_dir.name
    groupby_field = manifest.get("group_by_field") or manifest.get("groupby_field") or ""

    if root_type == "strategic_area" and "strategic_area" in parts:
        i = parts.index("strategic_area")
        if len(parts) > i + 1 and not strategic_area_id:
            strategic_area_id = parts[i + 1]
        if len(parts) > i + 2 and not run_id:
            run_id = parts[i + 2]
    elif root_type == "non_strategic" and "non_strategic" in parts:
        i = parts.index("non_strategic")
        if len(parts) > i + 1:
            non_strategic_groupby = parts[i + 1]
        if len(parts) > i + 2 and not run_id:
            run_id = parts[i + 2]

    return {
        "_source_file": str(path),
        "_source_rel_path": str(rel),
        "_source_run_dir": str(run_dir),
        "_source_root_type": root_type,
        "_source_modified_at": datetime.fromtimestamp(path.stat().st_mtime).isoformat(timespec="seconds") if path.exists() else None,
        "source_run_id": str(run_id),
        "source_groupby_field": str(groupby_field or non_strategic_groupby),
        "source_filter_fields_key": manifest.get("filter_fields_key", ""),
        "_inferred_strategic_area_id": str(strategic_area_id or ""),
        "_inferred_non_strategic_groupby": str(non_strategic_groupby or groupby_field or ""),
        "_run_manifest": manifest,
    }


def cc_find_curated_files(root: Path, include_strategic: bool = True, include_non_strategic: bool = True) -> list[Path]:
    """Find curated_df.csv files under NB03 strategic and non-strategic roots."""
    root = Path(root)
    roots: list[Path] = []
    if include_strategic:
        roots.append(root / "OUTPUTS" / "runs" / "strategic_area")
    if include_non_strategic:
        roots.append(root / "OUTPUTS" / "runs" / "non_strategic")
    matches: list[Path] = []
    for base in roots:
        if base.exists():
            matches.extend(sorted(base.rglob("curated_df.csv")))
    return sorted(dict.fromkeys(matches))


def cc_load_curated_union(root: Path, diagnostics: dict[str, Any] | None = None) -> pd.DataFrame:
    """Load every curated_df.csv and add stable source metadata."""
    diagnostics = diagnostics if diagnostics is not None else {}
    root = Path(root)
    files = cc_find_curated_files(root)
    diagnostics["curated_files_found"] = len(files)
    diagnostics.setdefault("read_errors", [])
    if not files:
        raise FileNotFoundError("No curated_df.csv files found under OUTPUTS/runs/strategic_area or OUTPUTS/runs/non_strategic")
    frames: list[pd.DataFrame] = []
    for path in files:
        try:
            df = pd.read_csv(path)
            meta = cc_infer_path_metadata(path, root)
            manifest = meta.pop("_run_manifest", {})
            for k, v in meta.items():
                if k not in df.columns or k.startswith("_") or k.startswith("source_"):
                    df[k] = v
            if "strategic_area_id" not in df.columns:
                df["strategic_area_id"] = meta.get("_inferred_strategic_area_id", "")
            else:
                df["strategic_area_id"] = df["strategic_area_id"].fillna("").astype(str)
                mask = df["strategic_area_id"].str.strip().eq("")
                df.loc[mask, "strategic_area_id"] = meta.get("_inferred_strategic_area_id", "")
            if "strategic_area_label" not in df.columns:
                df["strategic_area_label"] = df["strategic_area_id"].astype(str).str.replace("_", " ").str.title()
            if "run_date" not in df.columns and manifest.get("run_date"):
                df["run_date"] = manifest.get("run_date")
            frames.append(df)
        except Exception as e:
            diagnostics["read_errors"].append({"path": str(path), "error": f"{type(e).__name__}: {e}"})
    if not frames:
        raise RuntimeError("No curated_df.csv files could be read successfully")
    out = pd.concat(frames, ignore_index=True)
    out["global_insight_id"] = [cc_make_global_insight_id(row, i) for i, (_, row) in enumerate(out.iterrows(), start=1)]
    # The union script that builds insight_value_add_tiers.csv applies a
    # deterministic __dup_N suffix to the JOIN KEY itself when a base ID
    # collides. We have to do the same here, or duplicate rows on either
    # side silently merge to the wrong tier. html_insight_id is just an
    # alias for DOM uniqueness; it tracks the join key.
    dup_index = out.groupby("global_insight_id").cumcount()
    out["global_insight_id"] = out["global_insight_id"].where(
        dup_index.eq(0), out["global_insight_id"] + "__dup_" + dup_index.astype(str)
    )
    out["html_insight_id"] = out["global_insight_id"]
    diagnostics["curated_files_read_successfully"] = len(frames)
    diagnostics["input_insight_rows"] = int(len(out))
    diagnostics["duplicate_global_insight_ids"] = int(dup_index.gt(0).sum())
    return out


def cc_normalize_tier(value: Any) -> str:
    if value is None or (isinstance(value, float) and pd.isna(value)):
        return "Unscored"
    s = str(value).strip().upper()
    if s in {"A", "B", "C", "D"}:
        return s
    if s in {"", "NAN", "NONE", "NULL", "UNSCORED", "UNSCORE"}:
        return "Unscored"
    return "Unscored"


def cc_load_value_tiers(path: Path, diagnostics: dict[str, Any] | None = None) -> pd.DataFrame:
    """Load the sweep-review value-add tier file.

    Expected logical columns:
      - global_insight_id
      - tier

    The file may be comma-delimited or tab-delimited even if named .csv.
    Excel can display both cleanly, so this loader normalizes headers and
    explicitly retries common delimiters before failing.
    """
    path = Path(path)
    if not path.exists():
        if diagnostics is not None:
            cc_add_warning(
                diagnostics,
                "VALUE_TIER_MISSING",
                "Value-add tier CSV not found; all insights will be marked Unscored.",
                {"path": str(path)},
            )
        return pd.DataFrame(columns=["global_insight_id", "tier"])
    ...
    for kwargs in read_attempts:
        try:
            ...
            # If pandas read a tab-delimited file as one comma-delimited column,
            # the header may still be a single string containing tabs.
            if len(df.columns) == 1 and "\t" in df.columns[0]:
                continue

            if {"global_insight_id", "tier"}.issubset(set(df.columns)):
                best_df = df
                break
    ...
    if best_df is None:
        raise ValueError(
            f"Could not read value tier file: {path}. "
            f"Last error: {type(last_error).__name__}: {last_error}"
        )

    if "global_insight_id" not in best_df.columns or "tier" not in best_df.columns:
        raise ValueError(
            "Value tier file must contain global_insight_id and tier columns after header normalization. "
            f"Path: {path}. "
            f"Columns read: {list(best_df.columns)}"
        )

    out = best_df[["global_insight_id", "tier"]].copy()
    out["global_insight_id"] = out["global_insight_id"].astype(str).str.strip()
    out["tier"] = out["tier"].astype(str).str.strip().str.upper()
    out["tier"] = out["tier"].where(out["tier"].isin(["A", "B", "C", "D"]), "Unscored")

    # Drop blank IDs and deterministic duplicate rows.
    out = out[out["global_insight_id"].ne("")]
    out = out.drop_duplicates("global_insight_id", keep="first").reset_index(drop=True)

    if diagnostics is not None:
        diagnostics["value_tier_rows_read"] = int(len(out))
        diagnostics["value_tier_csv_path"] = str(path)

    return out


def cc_join_value_tiers(
    curated_df: pd.DataFrame,
    tier_df: pd.DataFrame,
    diagnostics: dict[str, Any],
) -> pd.DataFrame:
    """Join value-add tiers onto the unioned curated insight table.

    cc_load_value_tiers() returns columns:
      - global_insight_id
      - tier

    This function normalizes that raw tier column into the report-facing
    category_value_add_tier column expected by the rest of NB04.
    """
    out = curated_df.copy()

    if "global_insight_id" not in out.columns:
        raise ValueError("curated_df must contain global_insight_id before joining value tiers.")

    if tier_df.empty:
        out["category_value_add_tier"] = "Unscored"
    else:
        tier_work = tier_df.copy()

        # Support either raw tier files or already-normalized tier inputs.
        if "category_value_add_tier" not in tier_work.columns:
            if "tier" not in tier_work.columns:
                raise ValueError(
                    "tier_df must contain either 'tier' or 'category_value_add_tier'. "
                    f"Columns found: {list(tier_work.columns)}"
                )
            tier_work = tier_work.rename(columns={"tier": "category_value_add_tier"})

        tier_work = tier_work[["global_insight_id", "category_value_add_tier"]].copy()
        tier_work["global_insight_id"] = tier_work["global_insight_id"].astype(str).str.strip()
        tier_work["category_value_add_tier"] = tier_work["category_value_add_tier"].apply(cc_normalize_tier)
        tier_work = tier_work.drop_duplicates("global_insight_id", keep="first")

        out["global_insight_id"] = out["global_insight_id"].astype(str).str.strip()
        out = out.merge(tier_work, on="global_insight_id", how="left", validate="many_to_one")
        out["category_value_add_tier"] = out["category_value_add_tier"].apply(cc_normalize_tier)

    missing = int((out["category_value_add_tier"] == "Unscored").sum())
    diagnostics["missing_tier_count"] = missing

    if missing:
        cc_add_warning(
            diagnostics,
            "MISSING_VALUE_TIER",
            "Some insights did not match the value-add tier CSV and were marked Unscored.",
...
def cc_load_project_bridge_for_curated(curated_df: pd.DataFrame, root: Path, diagnostics: dict[str, Any] | None = None) -> pd.DataFrame:
    """Load insight_project_bridge.csv files and map local insight_id to global_insight_id."""
    diagnostics = diagnostics if diagnostics is not None else {}
    frames: list[pd.DataFrame] = []
    diagnostics.setdefault("warnings", [])
    if curated_df.empty or "_source_run_dir" not in curated_df.columns:
        return pd.DataFrame(columns=["global_insight_id", "project_id"])
    for run_dir, g in curated_df.groupby("_source_run_dir", dropna=False):
        run_dir_path = Path(str(run_dir))
        candidates = [run_dir_path / "insights" / "insight_project_bridge.csv", run_dir_path / "chart_data" / "insight_project_bridge.csv"]
        bridge_path = next((p for p in candidates if p.exists()), None)
        if bridge_path is None:
            diagnostics["warnings"].append({"code": "BRIDGE_MISSING", "message": "Missing insight_project_bridge.csv", "context": {"run_dir": str(run_dir_path)}})
            continue
        try:
            bridge = pd.read_csv(bridge_path)
            if "insight_id" not in bridge.columns or "project_id" not in bridge.columns:
                diagnostics["warnings"].append({"code": "BRIDGE_BAD_SCHEMA", "message": "Bridge missing insight_id/project_id", "context": {"path": str(bridge_path)}})
                continue
            map_cols = ["insight_id", "global_insight_id", "html_insight_id"]
            local_map = g[[c for c in map_cols if c in g.columns]].drop_duplicates("insight_id")
            bridge = bridge.merge(local_map, on="insight_id", how="left")
            bridge["_bridge_path"] = str(bridge_path)
            frames.append(bridge)
        except Exception as e:
            diagnostics["warnings"].append({"code": "BRIDGE_READ_ERROR", "message": f"Could not read bridge: {type(e).__name__}: {e}", "context": {"path": str(bridge_path)}})
    if not frames:
        return pd.DataFrame(columns=["global_insight_id", "project_id"])
    out = pd.concat(frames, ignore_index=True)
    return out[out["global_insight_id"].notna()].copy()


def cc_select_project_ids(row: pd.Series, bridge_lookup: dict[str, list[Any]], max_ids: int = 500) -> list[Any]:
    """Select the most relevant project IDs for charts/Looker."""
    candidates = [
        "top_project_ids", "top_project_ids_json", "top_500_project_ids", "supporting_project_ids", "project_ids", "project_id_list"
    ]
    for col in candidates:
        if col in row.index:
            ids = cc_parse_project_ids(row.get(col), max_ids=max_ids)
            if ids:
                return ids[:max_ids]
    ids = bridge_lookup.get(str(row.get("global_insight_id", "")), [])
    return ids[:max_ids]


def cc_build_bridge_project_lookup(bridge_df: pd.DataFrame, max_ids: int = 500) -> dict[str, list[Any]]:
    if bridge_df is None or bridge_df.empty or "global_insight_id" not in bridge_df.columns:
        return {}
    work = bridge_df.copy()
    sort_candidates = [c for c in ["topic_share", "weight", "mean_topic_share", "project_rank"] if c in work.columns]
    if sort_candidates:
        ascending = [False if c != "project_rank" else True for c in sort_candidates]
        work = work.sort_values(["global_insight_id"] + sort_candidates, ascending=[True] + ascending)
    out: dict[str, list[Any]] = {}
    for gid, g in work.groupby("global_insight_id", dropna=False):
        out[str(gid)] = cc_parse_project_ids(g["project_id"].tolist(), max_ids=max_ids)
    return out


def cc_build_looker_url(project_ids: list[Any], cfg: dict[str, Any]) -> str:
    """Build a Looker Explore URL from project IDs using params.yaml output settings."""
    out_cfg = cfg.get("output", {}) if isinstance(cfg, dict) else {}
    if not project_ids or not out_cfg.get("looker_base_url"):
        return ""
    limit_ids = int(out_cfg.get("looker_id_limit", 500) or 500)
    ids = [str(x).strip() for x in project_ids[:limit_ids] if str(x).strip()]
    if not ids:
        return ""
    params = {
        "fields": ",".join(out_cfg.get("looker_fields") or ["project.projectid", "project_essays.essay"]),
        "limit": str(out_cfg.get("looker_limit", limit_ids)),
        f"f[{out_cfg.get('looker_filter_field', 'project.projectid')}]":[",".join(ids)],
    }
    # urlencode handles list values if doseq=True.
    return str(out_cfg.get("looker_base_url")) + "?" + urlencode(params, doseq=True)


def cc_load_enriched_corpus(root: Path, diagnostics: dict[str, Any] | None = None) -> pd.DataFrame:
    """Load the prepared enriched parquet used for chart baselines."""
    diagnostics = diagnostics if diagnostics is not None else {}
    root = Path(root)
    candidates = [
    ...
    for path in candidates:
        if path.exists():
            ...
            if "state" not in df.columns:
                attr_path = root / "DATA" / "project_attributes.csv"
                if attr_path.exists():
                    try:
                        attrs = pd.read_csv(attr_path, usecols=["project_id", "state"]).drop_duplicates("project_id")
                        df = df.merge(attrs, on="project_id", how="left")
                    except Exception:
                        pass
            diagnostics["corpus_path"] = str(path)
            diagnostics["corpus_rows"] = int(len(df))
            return df
    diagnostics.setdefault("warnings", []).append({
        "code": "CORPUS_MISSING",
        "message": "Could not find enriched parquet; charts will be unavailable.",
        "context": {"searched": [str(p) for p in candidates]},
    })
    return pd.DataFrame()


def cc_counts_pct(series: pd.Series) -> pd.DataFrame:
    ...
    total = c["count"].sum()
    c["pct"] = c["count"] / total if total else 0.0
    return c


def cc_top_counts_pct(series: pd.Series, n: int = 8) -> pd.DataFrame:
    c = cc_counts_pct(series)
    if c.empty:
        return c
    return c.sort_values(["count", "category"], ascending=[False, True]).head(n).reset_index(drop=True)


def cc_prepare_chart_df(df: pd.DataFrame) -> pd.DataFrame:
    """Add the derived columns chart payloads expect (efs_category,
    posting_period, funding_status, project_cost_bucket).

    All derivations are vectorized; cost-of-prep is O(N) with no Python-level
    row apply. Important because this is called once on the full corpus (~6e5
    rows) and the old apply-based implementation was ~1000× slower than the
    per-insight slicing.
    """
    out = df.copy()
    if out.empty:
    ...
    if "efs_category" not in out.columns:
        def _yes_mask(col: str) -> pd.Series:
            if col not in out.columns:
                return pd.Series(False, index=out.index)
            return out[col].astype(str).str.strip().str.lower().eq("yes")

        rural = _yes_mask("school_is_underserved_rural")
        race = _yes_mask("school_is_historically_underrepresented_race")
        inc = _yes_mask("school_is_low_income")
        # Priority order: Race+Inc > Rural > Race > Inc > NonEFS.
    ...
    # ── posting_period (FY + half) ──────────────────────────────────
    if "posted_date" in out.columns:
        posted = pd.to_datetime(out["posted_date"], errors="coerce")
        yr = posted.dt.year
        mo = posted.dt.month
        # FY rolls Jul→Dec into the next calendar year.
        fy_year = yr.where(mo < 7, yr + 1) % 100
        fy_part = pd.Series(["Missing"] * len(out), index=out.index, dtype=object)
        valid = posted.notna()
        if valid.any():
            fy_part.loc[valid] = "FY" + fy_year.loc[valid].astype(int).astype(str).str.zfill(2)
        half_part = pd.Series([""] * len(out), index=out.index, dtype=object)
        if valid.any():
            half_part.loc[valid] = np.where(mo.loc[valid] >= 7, "H1", "H2")
        out["posting_period"] = (fy_part + " " + half_part).str.strip()

    # ── funding_status ─────────────────────────────────────────────
    if "funding_status" not in out.columns:
        funded = pd.to_datetime(out.get("funded_date", pd.Series(index=out.index)), errors="coerce").notna()
        exp = pd.to_datetime(out.get("expiration_date", pd.Series(index=out.index)), errors="coerce")
        expired = exp.notna() & (exp <= pd.Timestamp.today())
        out["funding_status"] = np.select([funded, (~funded) & expired], ["Funded", "Expired"], default="Live")

    # ── project_cost_bucket ────────────────────────────────────────
    if "project_cost_bucket" not in out.columns and "total_cost" in out.columns:
...
def cc_distribution_for_field(df: pd.DataFrame, field: str, resource_category_df: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    if field == "race":
        enroll = pd.to_numeric(df.get("school_enrollment", pd.Series(1, index=df.index)), errors="coerce").fillna(1)
        rows = []
        race_cols = {
        ...
        for label, col in race_cols.items():
            if col in df.columns and denom > 0:
                val = (pd.to_numeric(df[col], errors="coerce").fillna(0) * enroll).sum() / denom / 100.0
                rows.append({"label": label, "value": float(val)})
        return pd.DataFrame(rows)
    if field == "item_category":
        if resource_category_df is None or resource_category_df.empty or "project_id" not in df.columns:
            return pd.DataFrame(columns=["label", "value"])
        ids = set(df["project_id"].dropna().astype(str))
        r = resource_category_df[resource_category_df["project_id"].astype(str).isin(ids)].copy()
        if r.empty or "item_category" not in r.columns or "quantity_count" not in r.columns:
            return pd.DataFrame(columns=["label", "value"])
        top = r.groupby("item_category", dropna=True)["quantity_count"].sum().sort_values(ascending=False).head(8)
        total = r["quantity_count"].sum()
        return pd.DataFrame({"label": top.index.astype(str), "value": (top / total if total else top * 0).astype(float).values})
    col = col_map.get(field)
    if not col or col not in df.columns:
        return pd.DataFrame(columns=["label", "value"])
    dist = cc_top_counts_pct(df[col], n=12 if field == "state" else 8)
    return dist.rename(columns={"category": "label", "pct": "value"})[["label", "value"]]


CC_CHART_FIELDS = [
    "metro", "grade", "efs", "race", "item_category",
    "state", "posting", "cost_distr", "funding", "project_category",
]


def cc_chart_payload_for_ids(
    prepared_corpus: pd.DataFrame,
    project_ids: list[Any],
    *,
    baseline_cache: dict[str, pd.DataFrame] | None = None,
    resource_category_df: pd.DataFrame | None = None,
    fields: list[str] | None = None,
) -> tuple[dict[str, Any], int]:
    """Build compact chart payloads for one insight's project-id set.

    Args:
        prepared_corpus: The output of cc_prepare_chart_df(corpus). Must be
            already prepared — passing a raw corpus works but is slow when
            called per insight.
        project_ids:     The project IDs supporting this insight.
        baseline_cache:  Precomputed {field: distribution_df} for the full
            corpus. Avoid passing None when calling this in a per-insight
            loop; the baseline computation is O(corpus_size) per field.
        resource_category_df: Resource-category bridge for item_category
            charts. Pass None to skip item_category.
        fields:          Optional subset of CC_CHART_FIELDS to compute.

    Returns:
        (charts_dict, chart_project_count).
    """
    fields = fields or CC_CHART_FIELDS
    if prepared_corpus is None or prepared_corpus.empty or not project_ids:
        return {}, 0
    if "project_id" not in prepared_corpus.columns:
        return {}, 0
    ids = {str(x) for x in project_ids}
    df_i = prepared_corpus[prepared_corpus["project_id"].astype(str).isin(ids)]
    if df_i.empty:
        return {}, 0
    # If the caller passed a raw corpus by mistake, prep it now. Lazy
    # baseline if cache is missing.
    if "efs_category" not in df_i.columns or "posting_period" not in df_i.columns:
        prepared_corpus = cc_prepare_chart_df(prepared_corpus)
        df_i = prepared_corpus[prepared_corpus["project_id"].astype(str).isin(ids)]
    if baseline_cache is None:
        baseline_cache = {
            field: cc_distribution_for_field(prepared_corpus, field, resource_category_df=resource_category_df)
            for field in fields
        }

    charts: dict[str, Any] = {}
    for field in fields:
        dist = cc_distribution_for_field(df_i, field, resource_category_df=resource_category_df)
        if dist.empty:
            continue
        base = baseline_cache.get(field, pd.DataFrame(columns=["label", "value"]))
        base_map = dict(zip(base.get("label", []), base.get("value", [])))
        labels = dist["label"].astype(str).tolist()
        values = [float(v) if pd.notna(v) else 0.0 for v in dist["value"].tolist()]
        baseline = [float(base_map.get(lbl, 0.0) or 0.0) for lbl in labels]
        xmax = max(values + baseline + [0.01]) * 1.25
        charts[field] = {
            "labels": labels,
            "values": values,
            "baseline": baseline,
    ...
    return charts, int(df_i["project_id"].nunique())


def cc_build_baseline_cache(
    prepared_corpus: pd.DataFrame,
    *,
    resource_category_df: pd.DataFrame | None = None,
    fields: list[str] | None = None,
) -> dict[str, pd.DataFrame]:
    """Precompute full-corpus baseline distributions once for reuse across insights."""
    fields = fields or CC_CHART_FIELDS
    if prepared_corpus is None or prepared_corpus.empty:
        return {}
    return {
        field: cc_distribution_for_field(prepared_corpus, field, resource_category_df=resource_category_df)
        for field in fields
...
def cc_build_full_corpus_charts(
    ...
    *,
    ...
    """Compute the full-corpus chart shapes shown as baselines in the UI.

    Args:
        corpus_df:            Project-level corpus. Raw or pre-prepared.
        resource_category_df: Resource bridge for item_category baselines.
        baseline_cache:       Optional precomputed distributions from
            cc_build_baseline_cache(). When provided, skips redundant work.
        prepared:             True if corpus_df is already the output of
            cc_prepare_chart_df(); skips a second prep pass.
    """
    if corpus_df is None or corpus_df.empty:
        return {}
    full = corpus_df if prepared else cc_prepare_chart_df(corpus_df)
    if baseline_cache is None:
        baseline_cache = cc_build_baseline_cache(
            full, resource_category_df=resource_category_df
        )
    out: dict[str, Any] = {}
    for field, dist in baseline_cache.items():
        ...
        out[field] = {
            "labels": labels,
            "values": values,
            "xmax": round(max(values + [0.01]) * 1.25, 4),
        }
    return out


def cc_load_strategic_areas_resolved(
    cfg: dict[str, Any],
    root: Path,
    diagnostics: dict[str, Any] | None = None,
) -> dict[str, dict[str, Any]]:
    """Return the strategic_areas dict, resolving the external YAML if needed.

    Mirrors nb03 cell 4's resolution: prefer an inline cfg['strategic_areas']
    block; otherwise read the file at cfg['strategic_loop']['strategic_areas_path'].
    This is what lets the report nav show all 23 areas, including zero-count
    ones — the union of curated_df runs alone won't surface areas that haven't
    been run yet.

    Returns a dict shaped like {area_id: {"label": "...", ...}}. Empty dict
    when nothing can be resolved.
    """
    # 1) Inline first
    try:
        inline = get_strategic_areas_config(cfg)
    except Exception:
        inline = {}
    if inline:
        return inline
    ...
    # 2) External file
    sl = cfg.get("strategic_loop", {}) if isinstance(cfg, dict) else {}
    path_value = (sl or {}).get("strategic_areas_path")
    if not path_value:
        if diagnostics is not None:
            cc_add_warning(
                diagnostics,
                "STRATEGIC_AREAS_UNRESOLVED",
                "No inline strategic_areas in cfg and no strategic_loop.strategic_areas_path set. "
                "Zero-count strategic-area tabs cannot be rendered.",
                {},
            )
        return {}
    ...
    if not path.exists():
        if diagnostics is not None:
            cc_add_warning(
                diagnostics,
                "STRATEGIC_AREAS_PATH_MISSING",
                "strategic_loop.strategic_areas_path is set but the file is missing.",
                {"path": str(path)},
            )
        return {}
    try:
        ...
        if diagnostics is not None:
            cc_add_warning(
                diagnostics,
                "STRATEGIC_AREAS_PATH_READ_ERROR",
                f"Could not read strategic_areas YAML: {type(e).__name__}: {e}",
                {"path": str(path)},
            )
    return {}


def cc_value_tier_counts(curated: pd.DataFrame) -> dict[str, int]:
    """Return {tier_label: count} for joined-and-normalized curated_df.

    Always includes A, B, C, D, Unscored keys even if zero, so manifest
    diffs across runs are easy to read.
    """
    base = {"A": 0, "B": 0, "C": 0, "D": 0, "Unscored": 0}
    if curated is None or curated.empty or "category_value_add_tier" not in curated.columns:
        return base
    counts = curated["category_value_add_tier"].value_counts(dropna=False).to_dict()
    out = dict(base)
    for k, v in counts.items():
        key = cc_normalize_tier(k)
        out[key] = int(out.get(key, 0) + int(v))
    return out


def cc_looker_url_for_ids(project_ids: list[Any], cfg: dict[str, Any]) -> str:
    """Build a Looker URL for an insight's project IDs using the existing
    build_looker_project_url helper.

    Kept as a thin wrapper so the report builder shares URL semantics with
    nb03's docx builder — one implementation, one place to fix.
    """
    out_cfg = cfg.get("output", {}) if isinstance(cfg, dict) else {}
    base = out_cfg.get("looker_base_url")
    if not base or not project_ids:
        return ""
    try:
        return build_looker_project_url(
            base_url=str(base),
            project_ids=list(project_ids),
            filter_field=str(out_cfg.get("looker_filter_field", "project.projectid")),
            fields=out_cfg.get("looker_fields") or ["project.projectid", "project_essays.essay"],
            limit=int(out_cfg.get("looker_limit", 500) or 500),
            max_ids=int(out_cfg.get("looker_id_limit", 500) or 500),
        )
    except Exception:
        return ""


def cc_build_report_payload(
    *,
    root: Path,
    cfg: dict[str, Any],
    cfg_path: Path | str | None = None,
    value_tier_path: Path | None = None,
    max_project_ids: int | None = None,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Build the self-contained REPORT_DATA payload for the NB04 HTML report."""
    diagnostics: dict[str, Any] = {"warnings": []}
    root = Path(root)
    out_cfg = cfg.get("output", {}) if isinstance(cfg, dict) else {}
    max_project_ids = int(max_project_ids or out_cfg.get("csv_max_ids_per_insight", 500) or 500)
    tier_path = Path(value_tier_path) if value_tier_path else root / "OUTPUTS" / "sweep_review" / "insight_value_add_tiers.csv"

    # ── 1. Load + join curated insights with tiers ──────────────────────
    curated = cc_load_curated_union(root, diagnostics=diagnostics)
    tiers = cc_load_value_tiers(tier_path, diagnostics=diagnostics)
    curated = cc_join_value_tiers(curated, tiers, diagnostics=diagnostics)

    # ── 2. Project bridge + per-insight project ID selection ────────────
    bridge = cc_load_project_bridge_for_curated(curated, root=root, diagnostics=diagnostics)
    bridge_lookup = cc_build_bridge_project_lookup(bridge, max_ids=max_project_ids)

    curated["selected_project_ids"] = [
        cc_select_project_ids(row, bridge_lookup, max_ids=max_project_ids)
        for _, row in curated.iterrows()
    ]
    curated["looker_url_report"] = curated["selected_project_ids"].apply(
        lambda ids: cc_looker_url_for_ids(ids, cfg)
    )

    # ── 3. Load corpus + prepare ONCE, build baseline cache ONCE ────────
    # This is the critical perf fix. cc_prepare_chart_df on the full corpus
    # is expensive (it derives efs_category, posting_period, etc.). Doing it
    # once and reusing across all per-insight chart builds avoids the
    # O(insights × corpus_size) blow-up the old shape had.
    corpus = cc_load_enriched_corpus(root, diagnostics=diagnostics)
    resource_category_path = root / "DATA" / "project_resource_categorylevel.csv"
    resource_item_path = root / "DATA" / "project_resource_itemlevel.csv"
    resource_category_df = pd.read_csv(resource_category_path) if resource_category_path.exists() else pd.DataFrame()
    resource_item_df = pd.read_csv(resource_item_path) if resource_item_path.exists() else pd.DataFrame()

    prepared_corpus = cc_prepare_chart_df(corpus) if not corpus.empty else corpus
    baseline_cache = cc_build_baseline_cache(
        prepared_corpus, resource_category_df=resource_category_df
    )
    full_corpus = cc_build_full_corpus_charts(
        prepared_corpus,
        resource_category_df=resource_category_df,
        baseline_cache=baseline_cache,
        prepared=True,
    )

    # ── 4. Item-name lookup from selected relevant project IDs ──────────
    item_lookup: dict[str, list[list[Any]]] = {}
    if not resource_item_df.empty and "project_id" in resource_item_df.columns and "item_name" in resource_item_df.columns:
        for _, row in curated.iterrows():
            gid = str(row["global_insight_id"])
            ids = {str(x) for x in row["selected_project_ids"]}
            if not ids:
                continue
            sub = resource_item_df[resource_item_df["project_id"].astype(str).isin(ids)]
            if sub.empty:
                continue
            qty_col = "quantity_count" if "quantity_count" in sub.columns else None
            if qty_col:
                top = sub.groupby("item_name", dropna=True)[qty_col].sum().sort_values(ascending=False).head(10)
                item_lookup[gid] = [[str(k), int(v) if pd.notna(v) else 0] for k, v in top.items()]
            else:
                item_lookup[gid] = [
                    [str(x), 0]
                    for x in sub["item_name"].dropna().astype(str).value_counts().head(10).index
                ]

    # ── 5. Build per-insight payload entries ────────────────────────────
    insights: list[dict[str, Any]] = []
    chart_success = 0
    chart_missing = 0
    selected_id_union: set[str] = set()
    for _, row in curated.iterrows():
        gid = str(row["global_insight_id"])
        html_id = str(row.get("html_insight_id", gid))
        ids = row["selected_project_ids"] if isinstance(row["selected_project_ids"], list) else []
        selected_id_union.update(str(x) for x in ids)
        charts, chart_project_count = cc_chart_payload_for_ids(
            prepared_corpus,
            ids,
            baseline_cache=baseline_cache,
            resource_category_df=resource_category_df,
        ...
        source_root_type = str(row.get("_source_root_type", "unknown"))
        strat_id = str(row.get("strategic_area_id", "") or "").strip()
        if source_root_type == "non_strategic" or not strat_id:
            nav_area_id = "__non_strategic__"
            nav_area_label = "Other / Non-strategic"
            is_non_strategic = True
        else:
            nav_area_id = strat_id
            nav_area_label = str(row.get("strategic_area_label", strat_id.replace("_", " ").title()) or strat_id)
            is_non_strategic = False
        batch_core = str(row.get("split_id", "") or row.get("resolved_groupby_field", "") or row.get("source_groupby_field", "") or "batch")
        run_id = str(row.get("source_run_id", ""))
        short_run = run_id[:15] if run_id else "unknown run"
        batch_label = f"{nav_area_label} · {batch_core} · {short_run}" if batch_core else f"{nav_area_label} · {short_run}"

        tier = cc_normalize_tier(row.get("category_value_add_tier"))
        supporting = int(cc_safe_float(row.get("supporting_project_count", len(ids)), default=len(ids)))
        report_section = str(row.get("report_section", "") or row.get("section", "") or "")
        section_label = "Top" if "main" in report_section.lower() else "Secondary" if "append" in report_section.lower() else "Insight"

        insight = {
            "id": html_id,
            "global_insight_id": gid,
            "source_root_type": source_root_type,
            "source_run_id": run_id,
            "source_rel_path": str(row.get("_source_rel_path", "")),
            "batch_id": cc_report_slug(batch_label, max_len=120),
            "batch_label": batch_label,
            "strategic_area_id": nav_area_id,
            "strategic_area_label": nav_area_label,
            "raw_strategic_area_id": strat_id,
            "is_non_strategic": bool(is_non_strategic),
            "non_strategic_groupby_field": str(row.get("_inferred_non_strategic_groupby", "") or row.get("source_groupby_field", "")),
            "split_id": str(row.get("split_id", "") or row.get("resolved_groupby_field", "") or row.get("source_groupby_field", "")),
            "resolved_groupby_field": str(row.get("resolved_groupby_field", "") or row.get("source_groupby_field", "")),
            "category_bucket": str(row.get("category_bucket", "") or "Cross-category"),
            "category_value_add_tier": tier,
            "tier_sort": CC_REPORT_TIER_ORDER.get(tier, 5),
            "report_section": report_section,
            "section": str(row.get("section", "")),
            "section_label": section_label,
            "title": str(row.get("title", row.get("theme", gid)) or "").strip(),
            "finding": str(row.get("finding", row.get("what_we_see", "")) or "").strip(),
            "evidence_basis": str(row.get("evidence_basis", "") or "").strip(),
            "scope_or_caveat": str(row.get("scope_or_caveat", "") or "").strip(),
            "why_it_matters": str(row.get("why_it_matters", "") or "").strip(),
            "supporting_project_count": supporting,
            "project_count": supporting,
            "selected_project_count": len(ids),
            "chart_project_count": chart_project_count,
            "verified_topic_count": int(cc_safe_float(row.get("verified_topic_count", 0))),
            "claimed_topic_count": int(cc_safe_float(row.get("claimed_topic_count", 0))),
            "verification_ratio": cc_safe_float(row.get("verification_ratio", 0)),
            "mean_topic_share_all_verified_topics": cc_safe_float(row.get("mean_topic_share_all_verified_topics", 0)),
            "source_topics_verified": str(row.get("source_topics_verified", "") or ""),
            "top_project_ids": ids,
            "looker_url": str(row.get("looker_url", "") or row.get("looker_url_report", "") or ""),
            "item_names": item_lookup.get(gid, []),
            "charts": charts,
        }
    ...
    insights.sort(key=lambda x: (
        x["tier_sort"],
        0 if x["section_label"] == "Top" else 1,
        str(x.get("report_section", "")),
        -int(x.get("supporting_project_count", 0)),
        x.get("title", ""),
    ))

    # ── 6. Strategic-area nav: 23 areas plus Non-strategic ──────────────
    # Resolves the external CONFIG/strategic_areas.yaml if cfg only has the
    # path. This is what makes the zero-count tabs render.
    area_map: dict[str, str] = {}
    for ins in insights:
        area_map.setdefault(ins["strategic_area_id"], ins["strategic_area_label"])
    strat_cfg = cc_load_strategic_areas_resolved(cfg, root, diagnostics=diagnostics)
    for area_id, spec in (strat_cfg or {}).items():
        label = spec.get("label", area_id) if isinstance(spec, dict) else area_id
        area_map.setdefault(str(area_id), str(label))
    area_map.setdefault("__non_strategic__", "Other / Non-strategic")
    area_counts = pd.Series([i["strategic_area_id"] for i in insights]).value_counts().to_dict() if insights else {}
    strategic_areas = [
        {"id": aid, "label": label, "base_count": int(area_counts.get(aid, 0))}
        for aid, label in sorted(
            area_map.items(),
            # Pin non-strategic at the end of the tab strip; alpha-sort by label otherwise.
            key=lambda kv: (kv[0] == "__non_strategic__", kv[1]),
        )
    ]

    batch_counts = pd.Series([i["batch_id"] for i in insights]).value_counts().to_dict() if insights else {}
    batch_labels = {i["batch_id"]: i["batch_label"] for i in insights}
    batches = [
        {"id": bid, "label": batch_labels.get(bid, bid), "base_count": int(ct)}
        for bid, ct in sorted(batch_counts.items(), key=lambda kv: batch_labels.get(kv[0], kv[0]))
    ]

    # ── 7. Project counts: report the more honest number ───────────────
    # corpus_project_count is the unfiltered enriched parquet — the population
    # that backs the baseline charts. insight_project_count is the union of
    # selected project IDs across every rendered insight — the "X projects
    # supporting these insights" number that's actually meaningful in the UI.
    corpus_project_count = (
        int(corpus["project_id"].nunique())
        if not corpus.empty and "project_id" in corpus.columns
        else 0
    )
    insight_project_count = len(selected_id_union)

    report_title = out_cfg.get("report_title") or "Classroom Compass"
    payload = {
        "meta": {
            "run_id": "sweep_review_union",
            "run_date": datetime.now().strftime("%Y-%m-%d"),
            "report_title": report_title,
            # Hero number reflects the insight-supporting projects, not the
            # global enriched corpus. corpus_project_count is preserved for
            # the methods/trust panel if the template wants to surface it.
            "project_count": insight_project_count,
            "corpus_project_count": corpus_project_count,
            "insight_count": len(insights),
            "source_file_count": diagnostics.get("curated_files_read_successfully", 0),
            "value_tier_file": str(tier_path),
            "filter_headline": (
                f"{len(insights):,} insights from "
                f"{diagnostics.get('curated_files_read_successfully', 0):,} NB03 run files"
            ),
            "filter_detail": "Strategic and non-strategic runs combined; value-add tiers joined from sweep_review.",
            "filter_range": "Sweep review",
        },
        "account": {"name": report_title, "logo": "", "date": datetime.now().strftime("%Y-%m-%d")},
    ...
    # ── 8. Manifest diagnostics ────────────────────────────────────────
    # Per design doc section 4.3. Adds the fields that were silently missing,
    # most notably tier_counts (which the NB04 QA cell explicitly prints).
    diagnostics["final_insight_rows"] = int(len(insights))
    diagnostics["chart_payload_success_count"] = chart_success
    diagnostics["chart_payload_missing_count"] = chart_missing
    diagnostics["looker_url_success_count"] = int(sum(1 for i in insights if i.get("looker_url")))
    diagnostics["looker_url_missing_count"] = int(sum(1 for i in insights if not i.get("looker_url")))
    diagnostics["strategic_area_counts"] = {x["id"]: x["base_count"] for x in strategic_areas}
    diagnostics["batch_counts"] = {x["id"]: x["base_count"] for x in batches}
    diagnostics["tier_counts"] = cc_value_tier_counts(curated)
    diagnostics["non_strategic_count"] = int(
        sum(1 for i in insights if i.get("is_non_strategic"))
    )
    diagnostics["source_root_counts"] = (
        pd.Series([i["source_root_type"] for i in insights]).value_counts().to_dict()
        if insights else {}
    )
    diagnostics["curated_files_failed"] = int(len(diagnostics.get("read_errors", [])))
    diagnostics["source_files"] = sorted({
        i["source_rel_path"] for i in insights if i.get("source_rel_path")
    })
    diagnostics["corpus_project_count"] = corpus_project_count
    diagnostics["insight_project_count"] = insight_project_count
    diagnostics["created_at"] = datetime.now().isoformat(timespec="seconds")
    diagnostics["config_path"] = str(cfg_path) if cfg_path is not None else ""
    diagnostics["params_path"] = str(cfg_path) if cfg_path is not None else ""

    return payload, diagnostics


def cc_write_html_report(
    *,
    ...
    chartjs_path: Path | None = None,
) -> dict[str, Path]:
    """Inject REPORT_DATA into the HTML template and write report artifacts."""
    root = Path(root)
    template_path = Path(template_path)
    chartjs_path = Path(chartjs_path) if chartjs_path else root / "chart.umd.min.js"
    if not template_path.exists():
        raise FileNotFoundError(f"Template not found: {template_path}")
    if not chartjs_path.exists():
        raise FileNotFoundError(f"Chart.js not found: {chartjs_path}")
    output_dir = Path(output_dir) if output_dir else root / "OUTPUTS" / "sweep_review" / "reports"
    output_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%y%m%d_%H%M")
    data_path = output_dir / f"classroom_compass_report_data_{ts}.json"
    manifest_path = output_dir / f"classroom_compass_report_manifest_{ts}.json"
    html_path = output_dir / f"classroom_compass_report_{ts}.html"
    svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><circle cx="32" cy="32" r="30" fill="#FBBC04"/><polygon points="32,10 46,54 32,44 18,54" fill="#3E00C9"/></svg>'
    import base64
    ...
    report_json = json.dumps(payload, ensure_ascii=False, separators=(",", ":"), default=str)
    html = template.replace("__CHARTJS__", chartjs).replace("__FAVICON__", favicon).replace("__REPORT_DATA__", report_json)
    data_path.write_text(json.dumps(payload, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    manifest = {**diagnostics, "output_html_path": str(html_path), "output_report_data_path": str(data_path), "template_path": str(template_path)}
    manifest_path.write_text(json.dumps(manifest, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    html_path.write_text(html, encoding="utf-8")
    return {"html": html_path, "data": data_path, "manifest": manifest_path}
